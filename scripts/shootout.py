...
a REAL Syrup (fresh isolated home per run, your keys from .env) and scores it the
...
.syrup/shootout/ — publish it, and anyone can re-run it with their own keys.
...
from syrup.config import Settings, load_settings  # noqa: E402  (loads .env keys)
from syrup.ops.pricing import price_for  # noqa: E402
from syrup.ops.scoring import check_case, load_cases  # noqa: E402  (the ONE scorer)
...
def coding_shootout(runs: list[str], cases: list[dict], trials: int) -> str:
    ...
    from syrup.ops.coding_eval import run_coding_case
...
def main() -> None:
    ...
    if args.coding:
        from syrup.ops.coding_eval import load_coding_cases, pi_available
        if not pi_available():
