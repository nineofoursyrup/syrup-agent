"""Reset .syrup to a clean, curated state for a demo / recording.
...
What it does (your old state is backed up first, never just deleted):
  1. moves the current .syrup aside to .syrup.bak-<timestamp>
...
from syrup.tools.calendar import make_tool
...
FACTS = [
    ...
    ("user", "The user is building this project as a teaching repo and keeps notes here."),
...
def main(reset_spend: bool = False) -> None:
    ...
    print("  Run `syrup dashboard` and start filming.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reset .syrup to a clean demo state.")
    parser.add_argument("--yes", "-y", action="store_true",
                        help="required confirmation: yes, wipe .syrup (it is backed up first)")
    ...
    if not args.yes:
        ...
        print("REFUSING to run: demo_seed clears .syrup (memory, calendar, chat, traces"
