...
from syrup.memory.semantic.store import SqliteFactStore, _fts_query
...
def test_underscore_is_a_separator_here_because_it_is_one_in_the_index():
    """unicode61 splits on `_`, so `syrup_agent` is two terms in the index and
    has to be two terms here too, or it matches nothing."""
    assert _fts_query("syrup_agent") == "syrup OR agent"
