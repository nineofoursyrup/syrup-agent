...
is older than SYRUP_SESSION_IDLE_MINUTES, the next chat starts a new thread
(old one stays in History)."""

from __future__ import annotations

from evals.helpers import ScriptedClient, make_syrup
from syrup.ops.browser_agent import maybe_rotate_session
...
def test_idle_session_rotates(tmp_path, monkeypatch):
    monkeypatch.setenv("SYRUP_SESSION_IDLE_MINUTES", "60")
    app = make_syrup(tmp_path / "home", client=ScriptedClient([]))
...
def test_rotation_can_be_disabled(tmp_path, monkeypatch):
    monkeypatch.setenv("SYRUP_SESSION_IDLE_MINUTES", "0")
    app = make_syrup(tmp_path / "home", client=ScriptedClient([]))
    before = app.session.session_id
...
def test_provider_switch_resets_stale_model_overrides(tmp_path, monkeypatch):
    ...
    didn't newly type."""
    from syrup.ops import settings_api

    captured = {}
    monkeypatch.setenv("SYRUP_PROVIDER", "kimi")
    monkeypatch.setenv("SYRUP_MODEL", "kimi-k3")
    monkeypatch.setenv("SYRUP_SMALL_MODEL", "kimi-k3")
    ...
    try:
        ...
        pass
    assert captured.get("SYRUP_MODEL", "unset") in ("", "unset") or \
        captured.get("SYRUP_PROVIDER") == "gemini"
    # the actual contract: stale kimi ids must have been blanked
    assert captured.get("SYRUP_MODEL") != "kimi-k3"
    assert captured.get("SYRUP_SMALL_MODEL") != "kimi-k3"
