...
from syrup.loop.models import PROVIDERS, OpenAICompatClient, get_client
...
def fake_keys(monkeypatch):
    ...
    monkeypatch.delenv("SYRUP_API_KEY", raising=False)
    monkeypatch.delenv("SYRUP_BASE_URL", raising=False)
...
def test_dashboard_pricing_covers_every_provider(name):
    from syrup.ops.pricing import PRICING
...
def test_model_listing_falls_back_without_a_catalog(name, monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", name)
    monkeypatch.delenv("SYRUP_MODEL", raising=False)
    monkeypatch.delenv("SYRUP_SMALL_MODEL", raising=False)
...
def test_bad_key_gives_a_fixable_error_not_a_codec_crash(monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", "anthropic")
...
def test_catalog_url_is_used_with_both_auth_styles(monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", "kimi")
...
def test_price_for_layers_model_over_provider():
    ...
    from syrup.ops.pricing import MODEL_PRICING, PRICING, price_for
...
def test_every_priced_model_has_a_knowledge_cutoff():
    ...
    from syrup.ops.pricing import MODEL_CUTOFF, MODEL_PRICING, cutoff_for
...
# SYRUP_PROVIDER=xai printed "gate failed open (BadRequestError)". SYRUP_SMALL_MODEL
...
def test_env_model_names_do_not_leak_into_another_provider(monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", "anthropic")
    monkeypatch.setenv("SYRUP_MODEL", "claude-fable-5")
    monkeypatch.setenv("SYRUP_SMALL_MODEL", "claude-haiku-4-5-20251001")
...
def test_the_env_still_wins_for_its_own_provider(monkeypatch):
    """The whole point of SYRUP_MODEL is to override the default — the fix must
...
def test_a_foreign_gate_model_is_dropped_even_when_the_env_names_the_provider(monkeypatch):
    """The case that was actually live in a real .env: SYRUP_PROVIDER=xai with
    SYRUP_SMALL_MODEL still holding anthropic's gate model. Scoping by "did the
    caller switch provider" missed this one — the env agreed with itself and was
    still wrong."""
    from syrup.config import Settings
    from syrup.loop.models import get_client

    monkeypatch.setenv("SYRUP_PROVIDER", "xai")
    monkeypatch.setenv("SYRUP_SMALL_MODEL", "claude-haiku-4-5-20251001")
    monkeypatch.setenv("XAI_API_KEY", "test-key")
...
def test_an_unfamiliar_model_name_is_left_alone(monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", "kimi")
    monkeypatch.setenv("SYRUP_SMALL_MODEL", "moonshot-v1-8k")
