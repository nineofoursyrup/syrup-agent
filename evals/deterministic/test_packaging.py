...
1. THE WHEEL SHIPPED NO SKILLS. `pyproject.toml` packages `syrup`, but the
   bundled skills live in `skills/` at the REPO ROOT, so `pip install
   syrup-agent` produced a Syrup with zero procedural memory — one of the four
   pillars, silently absent. The dashboard shipped fine (it lives under
   `syrup/ops/static`), which is exactly why nobody noticed: the thing you can
...
2. THE TEST SUITE READ THE DEVELOPER'S `.env`. `syrup/config.py` calls
   load_dotenv() at import, so a stale `SYRUP_GRAPH_WORKFLOWS=1` routed every
...
def test_the_bundled_skills_are_findable():
    """Whatever the install shape, Syrup must find the skills it ships with."""
    dirs = bundled_skill_dirs()
    assert dirs, (
        "bundled_skill_dirs() found nothing — Syrup would start with no "
        "procedural memory and never say so"
    )
    for d in dirs:
        assert d.is_dir()
    names = {s.name for s in SkillLoader(dirs).skills}
    assert {"schedule-pin", "weekly-brief"} <= names, sorted(names)


def test_the_wheel_carries_the_skills_folder():
    """The build config half of the fix. `packages = ["syrup"]` alone leaves the
    repo-root skills/ out of the wheel, which is the bug — force-include copies
    it to syrup/skills so an installed Syrup finds it beside the code."""
    wheel = _pyproject()["tool"]["hatch"]["build"]["targets"]["wheel"]
    assert wheel.get("force-include", {}).get("skills") == "syrup/skills", (
        "pyproject no longer force-includes skills/ into the wheel — a "
        "`pip install syrup-agent` would ship with zero skills again"
    )


def test_lookup_covers_both_install_shapes():
    """The lookup half. A checkout has skills/ at the repo root; a wheel has it
    at syrup/skills. Exactly one exists at a time, so the function must look in
    both — checking only one is how this broke."""
    import inspect

    from syrup import memory
...
def test_evals_never_inherit_the_developers_env():
    """THE regression for #2. `make_syrup` must pin every switch that changes
    ...
    #28 split the pinning out of `make_syrup` into `settings_for` (so a test
    that builds `Syrup`, or a surface's own wiring function, directly — not
    through `make_syrup` — still gets it), so the source both functions
    combine to is what's checked here, not `make_syrup` alone."""
    import inspect

    from evals import helpers

    src = inspect.getsource(helpers.make_syrup) + inspect.getsource(helpers.settings_for)
    for switch in ("apple_calendar", "google_calendar", "apple_tools", "graph_workflows"):
        assert switch in src, (
            f"make_syrup/settings_for no longer pin {switch!r} — a stale value in "
            "the maintainer's .env can now change what these tests measure"
        )
...
def test_a_scripted_turn_ignores_the_graph_flag(tmp_path, monkeypatch):
    ...
    from evals.helpers import ScriptedClient, make_syrup, response, text_block
    ...
    monkeypatch.setenv("SYRUP_GRAPH_WORKFLOWS", "1")
