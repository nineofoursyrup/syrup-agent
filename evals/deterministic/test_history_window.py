...
Found while testing Telegram: its one always-on session accumulated
...
def test_prompt_history_is_windowed(tmp_path, monkeypatch):
    monkeypatch.setenv("SYRUP_HISTORY_TURNS", "3")   # keep only last 3 turns
    sent = []
    ...
    app = make_syrup(tmp_path / "home", client=Recorder(script))
    for i in range(5):
...
def test_default_window_is_generous_but_finite(tmp_path, monkeypatch):
    monkeypatch.delenv("SYRUP_HISTORY_TURNS", raising=False)
