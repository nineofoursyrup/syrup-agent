...
def test_google_calendar_settings_are_opt_in(monkeypatch):
    monkeypatch.delenv("SYRUP_GOOGLE_CALENDAR", raising=False)
    monkeypatch.delenv("SYRUP_GOOGLE_CALENDAR_ID", raising=False)
    assert Settings().google_calendar is False
    assert Settings().google_calendar_id == "primary"

    monkeypatch.setenv("SYRUP_GOOGLE_CALENDAR", "yes")
