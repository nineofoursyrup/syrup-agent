...
from syrup import integrations
from syrup.integrations import IntegrationState, IntegrationStatus
from syrup.loop.models import PROVIDERS
from syrup.ops import browser_agent
from syrup.tools import apple, calendar


def _isolate(monkeypatch, tmp_path):
    monkeypatch.setenv("SYRUP_HOME", str(tmp_path))
...
def _configure_google(monkeypatch, tmp_path):
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setenv("SYRUP_GOOGLE_CALENDAR", "1")
    monkeypatch.setenv("SYRUP_GOOGLE_CALENDAR_ID", "team@example.com")
    monkeypatch.setattr(integrations, "_extra_installed", lambda name: True)
...
def _configure_apple(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SYRUP_APPLE_CALENDAR", "1")
...
def test_apple_save_probes_before_recording_connected(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "apple_calendar", {"SYRUP_APPLE_CALENDAR": "1", "SYRUP_APPLE_CALENDARS": ""}
    )
...
def test_apple_force_save_records_error_without_probe(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "apple_calendar", {"SYRUP_APPLE_CALENDAR": "1"}, force=True
    )
...
def _configure_apple_tools(monkeypatch, tmp_path):
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setattr(integrations.sys, "platform", "darwin")
    monkeypatch.setenv("SYRUP_APPLE_TOOLS", "1")
...
def test_apple_tools_save_probes_and_records_connected(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "apple_tools", {"SYRUP_APPLE_TOOLS": "1"}
    )

    assert result.ok
    assert called == [True]
    assert os.environ["SYRUP_APPLE_TOOLS"] == "1"
    assert result.view is not None
    assert result.view.status.state is IntegrationState.CONNECTED
...
def test_apple_tools_force_save_skips_probe(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "apple_tools", {"SYRUP_APPLE_TOOLS": "1"}, force=True
    )
...
def test_google_save_probes_candidate_and_records_connected(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "google_calendar",
        {
            "SYRUP_GOOGLE_CALENDAR": "1",
            "SYRUP_GOOGLE_CALENDAR_ID": "candidate@example.com",
        },
    )
...
def test_google_save_failure_can_force_without_writing_first(monkeypatch, tmp_path):
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setenv("SYRUP_GOOGLE_CALENDAR", "")
    monkeypatch.setenv("SYRUP_GOOGLE_CALENDAR_ID", "old@example.com")
    monkeypatch.setattr(integrations, "_extra_installed", lambda name: True)
    monkeypatch.setattr(
        calendar,
        "probe_google_calendar",
    ...
    result = integrations.apply_integration(
        ...
        {
            "SYRUP_GOOGLE_CALENDAR": "1",
            "SYRUP_GOOGLE_CALENDAR_ID": "team@example.com",
        },
    )
    ...
    assert os.environ["SYRUP_GOOGLE_CALENDAR"] == ""
    assert os.environ["SYRUP_GOOGLE_CALENDAR_ID"] == "old@example.com"
...
def test_disabling_google_skips_probe(monkeypatch, tmp_path):
    ...
    result = integrations.apply_integration(
        "google_calendar", {"SYRUP_GOOGLE_CALENDAR": ""}
    )
