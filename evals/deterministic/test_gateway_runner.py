"""Background gateways keep each Syrup on one dedicated worker thread."""
...
from syrup.gateway import discord as discord_gateway
from syrup.gateway import telegram as telegram_gateway
from syrup.gateway.discord import DiscordHandle
from syrup.gateway.runner import (
    GATEWAY_ERROR_REPLY,
    GatewayAgentRunner,
    run_gateway_turn,
)
from syrup.gateway.telegram import TelegramHandle
from syrup.integrations import IntegrationState
...
def test_real_syrup_persists_two_telegram_turns_from_worker(tmp_path):
    home = tmp_path / "home"
    ...
    runner = GatewayAgentRunner(
        lambda: make_syrup(home, client=client, episodic_store="sqlite"),
        session_id="telegram",
        source="telegram",
    )
