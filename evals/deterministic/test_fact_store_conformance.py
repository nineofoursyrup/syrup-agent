...
`SupabaseFactStore` had two, and `SYRUP_SEMANTIC_STORE=supabase` therefore
...
whole point of syrup/memory/semantic/base.py.
...
from syrup.memory.semantic.base import FactStore
...
def _supabase_store(tmp_path):
    ...
    if os.getenv("SYRUP_TEST_SUPABASE") != "1":
        pytest.skip("set SYRUP_TEST_SUPABASE=1 (plus SUPABASE_* and OPENAI_API_KEY) to include it")
...
def _mem0_store(tmp_path):
    ...
    if os.getenv("SYRUP_TEST_MEM0") != "1":
        pytest.skip("set SYRUP_TEST_MEM0=1 (plus MEM0_API_KEY) to include it")
    from syrup.config import Settings
...
def _zep_store(tmp_path):
    if os.getenv("SYRUP_TEST_ZEP") != "1":
        pytest.skip("set SYRUP_TEST_ZEP=1 (plus ZEP_API_KEY) to include it")
    from syrup.config import Settings
...
def _langmem_store(tmp_path):
    ...
    measuring nothing."""
    if os.getenv("SYRUP_TEST_LANGMEM") != "1":
        pytest.skip("set SYRUP_TEST_LANGMEM=1 (plus OPENAI_API_KEY) to include it")
    from syrup.config import Settings
...
def test_a_blank_optional_field_falls_back_to_its_default(monkeypatch):
    ...
    """
    from syrup.memory.semantic.base import env_or

    monkeypatch.setenv("SYRUP_TEST_BLANK", "")
    assert env_or("SYRUP_TEST_BLANK", "fallback") == "fallback"
    assert float(env_or("SYRUP_TEST_BLANK", "2.0")) == 2.0

    monkeypatch.setenv("SYRUP_TEST_BLANK", "   ")
    assert env_or("SYRUP_TEST_BLANK", "fallback") == "fallback", "whitespace is blank too"

    monkeypatch.setenv("SYRUP_TEST_BLANK", "real")
    assert env_or("SYRUP_TEST_BLANK", "fallback") == "real"

    monkeypatch.delenv("SYRUP_TEST_BLANK")
    assert env_or("SYRUP_TEST_BLANK", "fallback") == "fallback"
...
def test_no_backend_reads_an_optional_setting_with_bare_os_getenv():
    ...
    semantic = Path(__file__).resolve().parents[2] / "syrup" / "memory" / "semantic"
    offenders = []
