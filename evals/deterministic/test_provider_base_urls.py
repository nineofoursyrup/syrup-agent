...
def test_catalog_follows_selected_provider_region(
    ...
    provider = PROVIDERS[provider_name]
    monkeypatch.setenv("SYRUP_PROVIDER", provider_name)
    monkeypatch.setenv(provider.key_env, "test-key")
    monkeypatch.setenv(base_url_env, selected_url)
    monkeypatch.delenv("SYRUP_BASE_URL", raising=False)
...
def test_apply_provider_persists_scoped_base_url_and_clears_legacy_override(
    ...
    (tmp_path / ".env").write_text("SYRUP_BASE_URL='https://legacy.example'\n")
    monkeypatch.setenv("SYRUP_HOME", str(tmp_path / ".syrup"))
    monkeypatch.setenv("SYRUP_PROVIDER", "minimax")
    monkeypatch.setenv("MINIMAX_API_KEY", "test-key")
    monkeypatch.setenv("SYRUP_BASE_URL", "https://legacy.example")
    ...
    assert os.environ.get("SYRUP_BASE_URL", "") == ""
...
def test_models_edit_modal_posts_selected_base_url():
    source = Path("syrup/ops/static/js/models.js").read_text()
...
def test_reusing_saved_provider_endpoint_skips_remote_probe_and_noop_rebuild(
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SYRUP_HOME", str(tmp_path / ".syrup"))
    monkeypatch.setenv("SYRUP_PROVIDER", current_provider)
...
def test_saving_noncurrent_provider_does_not_activate_or_rebuild(monkeypatch, tmp_path):
    ...
    assert result.ok
    assert os.environ["SYRUP_PROVIDER"] == "minimax"
