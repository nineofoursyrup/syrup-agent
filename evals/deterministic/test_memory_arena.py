...
from syrup.ops import memory_arena as arena
from syrup.ops.memory_arena import INVENTED, MISS, PASS, STALE
...
def test_inventing_a_deadline_that_was_never_given_is_invented():
    ...
    sets live outside this repo (see SYRUP_MEMORY_PROBES)."""
    probe = {"expect_refusal": True}
...
def test_retrieving_for_arithmetic_fails_even_with_the_right_number():
    ...
    behaviour, and only syrup can be graded on it — the gate is observable."""
...
def test_the_table_has_no_emojis():
    ...
    rows = arena.scoreboard([{"contestant": "syrup", "outcome": PASS}])
...
def test_the_shipped_fixture_is_only_an_example():
    """syrup ships the mechanism, not the questions. The bundled probes exist to
...
# the whole seed -> probe -> score path with a fake Syrup: no model, no keys, no
...
class _FakeSyrup:
    ...
    def __init__(self, settings=None, **kw):
        ...
        self._answers = dict(_FakeSyrup.script)
    ...
    def respond(self, message, source="cli", observer=None, **kw):
        ...
        if observer:
            observer("gate", {"decision": _FakeSyrup.gate})
...
def _run(monkeypatch, tmp_path, script, gate=False, backends=("sqlite",)):
    import syrup.app
    _FakeSyrup.script = script
    _FakeSyrup.gate = gate
    monkeypatch.setattr(syrup.app, "Syrup", _FakeSyrup)
    events = []
...
def test_a_backend_that_blows_up_does_not_take_the_others_with_it(monkeypatch, tmp_path):
    ...
    import syrup.app
    _FakeSyrup.script = {"q1?": "alpha", "q2?": "the new one"}
    _FakeSyrup.gate = False
    real_init = _FakeSyrup.__init__
    ...
    monkeypatch.setattr(_FakeSyrup, "__init__", explode)
    monkeypatch.setattr(syrup.app, "Syrup", _FakeSyrup)
    events = []
...
def test_the_gate_decision_reaches_the_scorer(monkeypatch, tmp_path):
    """Only syrup can be graded on 'did you search memory for arithmetic', and
    ...
    import syrup.app
    _FakeSyrup.script = {"q1?": "68"}
    _FakeSyrup.gate = True          # it retrieved when it should not have
    monkeypatch.setattr(syrup.app, "Syrup", _FakeSyrup)
    events = []
