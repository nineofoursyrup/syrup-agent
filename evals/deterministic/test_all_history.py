...
The Loop tab shows every turn across all threads, but the chat
dock only showed one thread at a time, so history felt missing. The
...
from syrup.ops.dashboard import _thread_history, session_action
...
def test_all_history_returns_every_thread(tmp_path, monkeypatch):
    monkeypatch.setenv("SYRUP_HOME", str(tmp_path / "home"))
