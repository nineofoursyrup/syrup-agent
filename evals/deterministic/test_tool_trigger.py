...
from evals.helpers import HAS_KEY, ScriptedClient, make_syrup, response, text_block, tool_block

RUN_LIVE_EVALS = os.getenv("SYRUP_RUN_LIVE_EVALS") == "1"
...
def test_create_event_writes_db_and_ics(tmp_path):
    ...
    app = make_syrup(tmp_path / "home", client=ScriptedClient([gate] + turn))
...
def test_iteration_guardrail_stops_runaway_loop(tmp_path):
    ...
    app = make_syrup(tmp_path / "home", client=ScriptedClient([gate] + runaway), max_iterations=3)
...
@pytest.mark.skipif(not (HAS_KEY and RUN_LIVE_EVALS),
                    reason="set SYRUP_RUN_LIVE_EVALS=1 and configure an API key to run live evals")
