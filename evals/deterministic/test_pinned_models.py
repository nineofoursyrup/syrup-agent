...
default (adopted when you switch to it). Live goal: "a default
...
The shortlist lives in .syrup/models.json as {"pinned": ["provider:model", ...]};
...
from syrup.ops import settings_api as d
...
def home(tmp_path, monkeypatch):
    ...
    # so SYRUP_PROVIDER must also be tracked to prevent leaking into later
...
def test_switching_provider_adopts_its_pinned_default(home, monkeypatch):
    ...
    monkeypatch.setenv("SYRUP_PROVIDER", "gemini")
    monkeypatch.setenv("SYRUP_MODEL", "gemini-3.5-flash")
    result = integrations.apply_provider("kimi")
    assert result.ok
    assert os.getenv("SYRUP_PROVIDER") == "kimi"
    assert os.getenv("SYRUP_MODEL") == "kimi-k3"  # not gemini's model
