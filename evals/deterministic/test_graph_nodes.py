...
from syrup.graph import END, START, Graph, run_graph
from syrup.graph.nodes import agent_node, llm_node, tool_node
...
def test_agent_node_forwards_cancellation_to_the_loop():
    """#26: a graph node is still a loop turn — should_cancel must reach the
    run_loop call inside agent_node exactly like it does outside a graph."""
    registry = ToolRegistry()
    calls: list[dict] = []
    registry.register(Tool(
        name="save_note", description="save a note",
    ...
    node = agent_node("full_agent", out_key="result", client=client,
                      model="main-model", system="You are a test agent.",
                      tools=registry, should_cancel=lambda: True)

    state = run_graph(one_node_graph(node), {"message": "note milk"})

    assert calls == []                        # cancelled before the model was even asked
    assert state["result"].cancelled is True


def test_agent_node_forwards_approval_to_the_loop():
    """#26: a tool marked needs_approval must still be asked about when the
    node runs inside a graph — the approver is not a plain-loop-only thing."""
    registry = ToolRegistry()
    calls: list[dict] = []
    registry.register(Tool(
        name="save_note", description="save a note",
    ...
    assert state["result"].reply == "ok, not saved"
