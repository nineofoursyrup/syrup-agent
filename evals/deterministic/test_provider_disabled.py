...
def _isolate(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("SYRUP_DISABLED_PROVIDERS", raising=False)
    monkeypatch.delenv("SYRUP_PROVIDER", raising=False)


def test_disabled_providers_parsing(monkeypatch):
    monkeypatch.setenv("SYRUP_DISABLED_PROVIDERS", "openai, minimax,,xai")
    assert Settings().disabled_providers == frozenset({"openai", "minimax", "xai"})
    monkeypatch.delenv("SYRUP_DISABLED_PROVIDERS")
...
def test_toggle_persists_and_roundtrips(monkeypatch, tmp_path):
    ...
    assert os.environ["SYRUP_DISABLED_PROVIDERS"] == "openai"
    # dotenv.set_key quotes the value — verify it roundtrips correctly instead
    # of asserting the exact format.
    assert "SYRUP_DISABLED_PROVIDERS" in (tmp_path / ".env").read_text()
    from dotenv import dotenv_values
    assert dotenv_values(tmp_path / ".env")["SYRUP_DISABLED_PROVIDERS"] == "openai"
    assert integrations.apply_provider_disabled("openai", disabled=False).ok
    assert os.environ["SYRUP_DISABLED_PROVIDERS"] == ""
...
def test_toggle_rejects_unknown_and_current(monkeypatch, tmp_path):
    ...
    assert os.environ.get("SYRUP_DISABLED_PROVIDERS", "") == ""
...
def test_settings_info_reports_disabled(monkeypatch, tmp_path):
    _isolate(monkeypatch, tmp_path)
    monkeypatch.setenv("SYRUP_DISABLED_PROVIDERS", "glm")
    from syrup.ops.settings_api import settings_info

    assert settings_info()["disabled_providers"] == ["glm"]


def test_every_provider_has_a_logo():
    logos = Path(__file__).resolve().parents[2] / "syrup" / "ops" / "static" / "logos"
