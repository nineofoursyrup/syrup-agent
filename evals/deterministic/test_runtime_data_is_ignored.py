...
syrup runs on people's own machines with their own keys, and it writes a lot of
...
The gap this was written for (2026-07-26): .gitignore listed `.syrup/` by exact
...
second home was called `.syrup-discord/`, matched nothing, and its SOUL.md,
usage.jsonl and outbox/ were tracked. A privacy fix had quietly created a
different privacy hole. `.syrup-*/` closes it for any alternate home.

Prefix rules only protect the homes you can name in advance, so SOUL.md,
usage.jsonl and outbox/ are ignored by FILENAME too — a home is private
because of what it holds, not what its directory is called.
...
# alternate one (DISCORD_HOME, or any SYRUP_HOME a user points elsewhere).
ARTIFACTS = ["state.db", "SOUL.md", "usage.jsonl", "calendar.ics",
             "traces/2026-07-26.jsonl", "outbox/msg-1.txt"]


@pytest.mark.parametrize("home", [".syrup", ".syrup-discord", ".syrup-demo"])
...
def test_secrets_are_ignored(secret):
    ...
    ignore rule is load-bearing rather than belt-and-braces. This one was found
    missing on 2026-07-26."""
...
def test_the_env_template_carries_no_real_values():
    ...
    # SYRUP_PROVIDER is a non-secret default and is meant to have a value.
    unexpected = [ln for ln in filled if not ln.startswith("SYRUP_PROVIDER=")]
