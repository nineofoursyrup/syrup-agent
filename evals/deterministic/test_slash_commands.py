...
from syrup.ops import commands
...
def test_triage_is_a_command_and_takes_a_message():
    ...
    fn = importlib.import_module("syrup.ops.triage").run_triage
...
def test_discovery_requires_both_halves():
    ...
    syrup/ops/ is what makes it runnable, so that is what earns the command."""
    import pkgutil

    import syrup.graph.workflows as pkg
    ...
    for name, target in commands.discover().items():
        ...
        assert target == f"syrup.ops.{name}:run_{name}"
...
def test_the_dashboard_no_longer_says_you_never_pick_a_mode():
    ...
    js = pathlib.Path(__file__).resolve().parents[2] / "syrup/ops/static/js/views.js"
...
def test_a_runner_without_a_message_parameter_is_not_given_one():
    ...
    fn = importlib.import_module("syrup.ops.gather").run_gather
