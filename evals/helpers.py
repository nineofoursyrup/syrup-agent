...
real-Syrup factory for live ones."""
...
def _has_key() -> bool:
    """True when the ACTIVE provider (SYRUP_PROVIDER) has its key set, so live
...
class ScriptedClient:
    ...
    def remaining(self) -> int:
        """How many scripted responses were never consumed — lets a test
        prove a model call it expected NOT to happen really didn't."""
        return len(self._script)


def settings_for(home: Path, **settings_overrides):
    """Build a `Settings` for an isolated home dir. Split out of `make_syrup`
    so a test that needs to construct `Syrup` (or a surface's own wiring
    function, like `syrup.gateway.cli._build_syrup`) itself — rather than
    through `make_syrup` — still gets the same safe, deterministic pinning.

    A test must describe its own world. `syrup/config.py` calls load_dotenv()
    ...
    turn DOES, pinned off unless a test asks for it:
      ...
      graph_workflows        route every message through the triage graph,
                             ...
                             a stale SYRUP_GRAPH_WORKFLOWS=1 ate a scripted
    ...
    return Settings(home=home, **settings_overrides)


def make_syrup(home: Path, client=None, **settings_overrides):
    """Build a Syrup with an isolated home dir; optionally swap in a fake client."""
    from syrup.app import Syrup

    settings = settings_for(home, **settings_overrides)
    if client is not None and not settings.api_key:
        settings.api_key = "offline"  # never read the real key for scripted runs
    return Syrup(settings=settings, client=client)
