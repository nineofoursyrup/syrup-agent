...
from syrup.loop.models import PROVIDERS, Provider
from syrup.memory.episodic.notion_store import normalize_database_id
...
def _darwin(env: Mapping[str, str]) -> bool:
    return sys.platform == "darwin" and bool(env.get("SYRUP_APPLE_CALENDAR"))


def _darwin_tools(env: Mapping[str, str]) -> bool:
    return sys.platform == "darwin" and bool(env.get("SYRUP_APPLE_TOOLS"))


INTEGRATIONS: tuple[Integration, ...] = (
    Integration("dingtalk", "Channels", "DingTalk", "Lets Syrup receive and send DingTalk (钉钉) messages.",
                (EnvField("DINGTALK_CLIENT_ID", "Client ID", required=True, secret=True),
                 EnvField("DINGTALK_CLIENT_SECRET", "Client secret", required=True, secret=True),
                 EnvField("DINGTALK_ALLOWED_USER", "Allowed user"),
                 EnvField("DINGTALK_HOME", "Workspace directory")), "dingtalk", "dingtalk_stream",
                "https://open-dev.dingtalk.com", ReloadMode.GATEWAY,
                lambda env: bool(env.get("DINGTALK_CLIENT_ID")), None),
    Integration("feishu", "Channels", "Feishu", "Lets Syrup receive and send Feishu (飞书/Lark) messages.",
                (EnvField("FEISHU_APP_ID", "App ID", required=True, secret=True),
                 EnvField("FEISHU_APP_SECRET", "App secret", required=True, secret=True),
                 EnvField("FEISHU_ALLOWED_USER", "Allowed user"),
                 EnvField("FEISHU_HOME", "Workspace directory")), "feishu", "lark_oapi",
                "https://open.feishu.cn/app", ReloadMode.GATEWAY,
                lambda env: bool(env.get("FEISHU_APP_ID")), None),
    Integration("telegram", "Channels", "Telegram", "Lets Syrup receive and send Telegram messages.",
    ...
    Integration("discord", "Channels", "Discord", "Lets Syrup answer in a Discord server.",
                ...
                _positive_int),
    Integration("whatsapp", "Channels", "WhatsApp", "Lets Syrup answer WhatsApp messages via the Meta Cloud API.",
    ...
    Integration("google_calendar", "Calendar & Productivity", "Google Calendar",
                "Lets Syrup create and update Google Calendar events.",
                (EnvField("SYRUP_GOOGLE_CALENDAR", "Enable Google Calendar", FieldKind.BOOL),
                 EnvField("SYRUP_GOOGLE_CALENDAR_ID", "Calendar ID", default="primary")),
                "gcal", "googleapiclient", "https://developers.google.com/calendar/api/quickstart/python",
                ReloadMode.AGENT, lambda env: bool(env.get("SYRUP_GOOGLE_CALENDAR")), None),
    Integration("apple_calendar", "Calendar & Productivity", "Apple Calendar",
                "Lets Syrup work with Apple Calendar on this Mac.",
                (EnvField("SYRUP_APPLE_CALENDAR", "Enable Apple Calendar", FieldKind.BOOL),
                 EnvField("SYRUP_APPLE_CALENDARS", "Calendars")), None, None, "", ReloadMode.AGENT,
                _darwin, None),
    Integration("apple_tools", "Calendar & Productivity", "Apple Tools",
                "Lets Syrup use Apple Mail and other local Apple tools.",
                (EnvField("SYRUP_APPLE_TOOLS", "Enable Apple tools", FieldKind.BOOL),), None, None,
                "", ReloadMode.AGENT, _darwin_tools, None),
    Integration("notion", "Memory & Storage", "Notion", "Stores episodic memory in a Notion database.",
                (EnvField("SYRUP_EPISODIC_STORE", "Episodic store", FieldKind.CHOICE,
                ...
                lambda env: env.get("SYRUP_EPISODIC_STORE") == "notion", None, _notion_normalize),
    Integration("mem0", "Memory & Storage", "Mem0", "Stores semantic memory in the Mem0 service.",
                (EnvField("SYRUP_SEMANTIC_STORE", "Semantic store", FieldKind.CHOICE,
                 ...
                 EnvField("MEM0_API_KEY", "API key", required=True, secret=True,
                          help="From app.mem0.ai. The adapter sends infer=False so add() always "
                               ...
                               "own extraction step."),
                 EnvField("MEM0_USER_ID", "User id", help="Defaults to 'syrup'. Change it only if "
                                                          "several people share one Mem0 account.")),
                "arena", "mem0", "", ReloadMode.AGENT,
                lambda env: env.get("SYRUP_SEMANTIC_STORE") == "mem0", None),
    Integration("zep", "Memory & Storage", "Zep", "Stores semantic memory in a Zep temporal graph.",
                (EnvField("SYRUP_SEMANTIC_STORE", "Semantic store", FieldKind.CHOICE,
                 ...
                 EnvField("ZEP_USER_ID", "User id", help="Defaults to 'syrup'. Zep scopes a graph per user."),
                 EnvField("ZEP_MAX_WAIT_SECONDS", "Max wait (s)",
                          help="Ingestion is asynchronous: graph.add returns in ~0.2s with the episode "
                               ...
                               "nodes and edges. Syrup polls until it has, up to this long. Default 120 — "
                               "raise it if seeding times out, never lower it to make a benchmark finish.")),
                "arena", "zep_cloud", "", ReloadMode.AGENT,
                lambda env: env.get("SYRUP_SEMANTIC_STORE") == "zep", None),
    Integration("langmem", "Memory & Storage", "LangMem",
                ...
                (EnvField("SYRUP_SEMANTIC_STORE", "Semantic store", FieldKind.CHOICE,
                 ...
                 EnvField("SYRUP_LANGMEM_POSTGRES", "Postgres URL",
                ...
                lambda env: env.get("SYRUP_SEMANTIC_STORE") == "langmem", None),
    Integration("supabase", "Memory & Storage", "Supabase", "Stores semantic memory in Supabase pgvector.",
                ...
                lambda env: env.get("SYRUP_SEMANTIC_STORE") == "supabase", None),
    Integration("tavily", "Search & Observability", "Tavily", "Lets Syrup search the web.",
...
# T2 implementation: the cache path mirrors syrup.ops.catalog's .syrup storage.
...
def _health_path() -> Path:
    # Keep CLI and Dashboard aligned with the rest of Syrup's runtime files.
...
def render_env_example_block() -> str:
    ...
    lines = [
        ...
        "# Generated by scripts/generate_env_example.py; definitions live in syrup/integrations.py",
        "",
        "SYRUP_PROVIDER=anthropic",
    ]
    group = ""
    for integration in registry():
...
def _notion_probe(values: Mapping[str, str]) -> None:
    from syrup.memory.episodic.notion_store import NotionEpisodeStore
...
def _google_calendar_probe(values: Mapping[str, str]) -> None:
    from syrup.config import load_settings
    from syrup.tools import calendar

    calendar.probe_google_calendar(
        load_settings().home,
        values.get("SYRUP_GOOGLE_CALENDAR_ID", "") or "primary",
    )
...
def apply_integration(key: str, values: Mapping[str, str], clear: tuple[str, ...] = (), *, force: bool = False) -> ApplyResult:
    ...
    try:
        ...
        if integration.reload is ReloadMode.AGENT:
            from syrup.ops import browser_agent
...
def apply_provider(provider: str, *, key: str | None = None, model: str | None = None,
    ...
    from syrup.ops import browser_agent, catalog

    previous = os.environ.get("SYRUP_PROVIDER", "")
    selected = PROVIDERS[provider]
    switching = activate and provider != previous
    updates: dict[str, str] = {"SYRUP_PROVIDER": provider} if activate else {}
    if model is not None:
        updates["SYRUP_MODEL"] = model
    elif switching:
        updates["SYRUP_MODEL"] = catalog.default_model_for(provider)
    if small_model is not None:
        updates["SYRUP_SMALL_MODEL"] = small_model
    elif switching:
        updates["SYRUP_SMALL_MODEL"] = ""
    if key:
    ...
    # after a switch.  A legacy SYRUP_BASE_URL for the current provider is
    # migrated the next time its endpoint is saved.
    if selected.base_url_env and (base_url is not None or switching):
        legacy = os.environ.get("SYRUP_BASE_URL", "") if provider == previous else ""
        ...
        updates["SYRUP_BASE_URL"] = ""
    elif base_url is not None:
        updates["SYRUP_BASE_URL"] = base_url
    if custom_key is not None:
        updates["SYRUP_API_KEY"] = custom_key
    ...
    legacy_base_url = os.environ.get("SYRUP_BASE_URL", "") if provider == previous else ""
    current_base_url = legacy_base_url or selected.configured_base_url() or ""
    candidate_base_url = (
        updates.get(selected.base_url_env, current_base_url)
        if selected.base_url_env else updates.get("SYRUP_BASE_URL", current_base_url)
    )
    ...
    try:
        ...
        if candidate_key and (key_changed or base_url_changed) and not force:
            probe_names = {"SYRUP_PROVIDER", selected.key_env}
            if selected.base_url_env:
                probe_names.update({selected.base_url_env, "SYRUP_BASE_URL"})
            probe_before = {name: os.environ.get(name) for name in probe_names}
            os.environ["SYRUP_PROVIDER"] = provider
            ...
            if selected.base_url_env and selected.base_url_env in updates:
                ...
                os.environ["SYRUP_BASE_URL"] = ""
            elif base_url is not None:
                os.environ["SYRUP_BASE_URL"] = base_url
            try:
...
def apply_provider_disabled(provider: str, *, disabled: bool) -> ApplyResult:
    ...
    if provider not in PROVIDERS:
        return ApplyResult(False, error="unknown provider")
    if disabled and os.environ.get("SYRUP_PROVIDER", "") == provider:
        return ApplyResult(False, error="cannot disable the current provider — switch to another provider first")
    current = {p.strip() for p in os.environ.get("SYRUP_DISABLED_PROVIDERS", "").split(",") if p.strip()}
    updated = (current | {provider}) if disabled else (current - {provider})
    _write_updates({"SYRUP_DISABLED_PROVIDERS": ",".join(sorted(updated))}, ())
    return ApplyResult(True, _current_view(provider))
