"""MCP connector — plug any Model Context Protocol server into Syrup's tools.

Syrup's loop is synchronous; the MCP SDK is async. The bridge below runs one
...
class MCPBridge:
    ...
    def start(self) -> list[Tool]:
        ...
        for srv, metas in listed.items():
            for meta in metas:
                tools.append(self._build_tool(srv, meta))
        return tools

    def _build_tool(self, server: str, meta: dict) -> Tool:
        """One MCP tool listing -> one registry Tool. Split out from start()
        so it's testable without a live server (#24's regression eval).
        needs_approval defaults True here, deliberately not overridable per
        server: this is third-party code with a schema this repo did not
        write and an implementation it has never read, and for code you
        haven't read, asking before every call is the only honest default."""
        return Tool(
            name=f"{server}_{meta['name']}",
            description=f"[MCP:{server}] {meta.get('description','') or ''}",
            input_schema=meta.get("inputSchema") or {"type": "object", "properties": {}},
            fn=(lambda server=server, tname=meta["name"], **kw: self.call(server, tname, kw)),
            needs_approval=True,
        )
