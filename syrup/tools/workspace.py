...
dir — so each scratch delegation instead lands in
    ...
    <SYRUP_WORKSPACE>/<YYYY-MM-DD>/<HHMMSS>-<model>-<slug>/
        <the files pi wrote>
...
lives OUTSIDE .syrup and is git-ignored — it never pollutes the agent's real
state or the repo.
...
WORKSPACE_ENV = "SYRUP_WORKSPACE"          # root dir; default ./syrup_workspace
AUTORUN_ENV = "SYRUP_DELEGATE_AUTORUN"     # "0"/"false"/"no" to disable auto-run
RUN_TIMEOUT = int(os.getenv("SYRUP_AUTORUN_TIMEOUT", "30"))
_OURS = {"MANIFEST.md", "run.log", "pi-transcript.log"}
_ENTRY_PREFS = ("main.py", "app.py", "run.py", "game.py")


def workspace_root() -> Path:
    return Path(os.getenv(WORKSPACE_ENV, "syrup_workspace")).expanduser().resolve()
