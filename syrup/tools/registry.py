...
class Tool:
    ...
    # Static, declared here at registration time — same treatment as
    # wants_notify, kept out of to_api() so the model never sees it. True
    # means the loop's approver (#24) is consulted immediately before every
    # dispatch of this tool; a decline skips execute() entirely. False (the
    # default) means it runs straight through, same as before #24 existed.
    # MCP-registered tools (mcp_client.py) default this True: they're
    # third-party code with implementations this repo has never read, and
    # for code you haven't read, asking is the only honest default.
...
class ToolRegistry:
    ...
    def requires_approval(self, name: str) -> bool:
        """Whether `name` is a registered tool whose static declaration
        requires the loop's approver to be consulted before dispatch.
        Unknown names read as False — they'll hit the existing 'unknown
        tool' error from execute() same as always; approval was never in
        play for a tool that doesn't exist."""
        tool = self._tools.get(name)
        return bool(tool and tool.needs_approval)
