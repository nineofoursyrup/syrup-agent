...
Apple ecosystem (SYRUP_APPLE_TOOLS=1) and MCP servers (.syrup/mcp.json)."""
...
def build_registry(conn: sqlite3.Connection, settings: Settings, memory=None) -> ToolRegistry:
    ...
    # source (Google when signed in, plus syrup's own), so the model never has
    ...
    # Experimental tools — off by default; opt in with SYRUP_EXPERIMENTAL=1.
    ...
    # SYRUP_EXPERIMENTAL, so re-checking the env here would let the global switch
    # override an explicit False — and the arena passes experimental=False for
    # every non-coding race. Once the dashboard could write SYRUP_EXPERIMENTAL=1,
    # that OR silently forced delegate_task into races that never asked for it.
    if getattr(settings, "experimental", False):
        from syrup.tools import experimental
    ...
    # MCP servers (opt-in via .syrup/mcp.json).
    ...
    if mcp_config.exists():
        try:
            ...
            for t in bridge.start():
                registry.register(t)
            registry.mcp_bridge = bridge  # so Syrup.close() can stop the servers
        except ImportError:
            print("mcp.json found but the 'mcp' package is missing — pip install 'syrup-agent[mcp]'")
