...
Where events land:
  ...
  opt-in      Apple Calendar, in a dedicated "Syrup" calendar, via AppleScript —
              set SYRUP_APPLE_CALENDAR=1. First use makes macOS ask permission
              for your terminal to control Calendar; approve once.
  opt-in      Google Calendar via SYRUP_GOOGLE_CALENDAR=1. Local files remain
...
relays it, so Syrup never over-claims what happened.
"""
...
def _record_apple_calendar_health(ok: bool, message: str) -> None:
    ...
    try:
        from syrup.integrations import IntegrationState, IntegrationStatus, record_health
...
def sync_to_apple_calendar(title: str, start: str, end: str, notes: str = "") -> str:
    """Create the event in Calendar.app under the 'Syrup' calendar (created on
    ...
    # Prefer a dedicated "Syrup" calendar, but macOS can't create calendars in
...
def make_tool(
    ...
    def create_event(
        ...
        if not apple_calendar and not google_calendar:
            where += (
                " Not synced to any calendar app (enable with SYRUP_APPLE_CALENDAR=1 "
                "or SYRUP_GOOGLE_CALENDAR=1, "
...
def make_list_tool(conn: sqlite3.Connection, home: Path | None = None) -> Tool:
    ...
    syrup itself created. Every source is LABELLED in the output, so the agent can
    ...
    def list_events(start: str = "", end: str = "", limit: int = 20) -> str:
        ...
        if home is not None:
            from syrup.tools.google_calendar import is_connected, list_google_events
        ...
        checked.append("syrup's local calendar")
        local = local_events(start, end, limit)
        if local:
            sections.append("From syrup's local calendar (events syrup created):\n" + local)
    ...
    return Tool(
        ...
        description=(
            ...
            "when signed in, plus syrup's own local calendar). "
