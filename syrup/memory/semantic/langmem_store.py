"""Semantic memory via LangGraph's store — the LangMem backend.
    ...
    pip install 'syrup-agent[arena]'
    SYRUP_SEMANTIC_STORE=langmem   OPENAI_API_KEY=sk-...   # embeddings
    SYRUP_LANGMEM_POSTGRES=postgresql://...                # optional, see below
...
pillars Syrup's whiteboard has taught since 2026-06-19, arrived at independently
...
Without SYRUP_LANGMEM_POSTGRES this uses InMemoryStore, which LangGraph's own
...
returning nothing is the store working as documented, not a bug in Syrup.
...
class LangMemFactStore:
    ...
    def _make_store():
        ...
        dsn = os.getenv("SYRUP_LANGMEM_POSTGRES", "").strip()
        if dsn:
