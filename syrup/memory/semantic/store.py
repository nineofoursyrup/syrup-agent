...
Want vectors? Set SYRUP_SEMANTIC_STORE=supabase (see supabase_store.py).
"""
...
def _fts_query(text: str) -> str:
    ...
    index, and the index was not the side that was wrong:
      ...
      * every non-Latin script reduced to "" — and an empty query is not a
        ...
        handed to the model under the heading "Relevant memory". Syrup wasn't
    ...
    unicode61 treats it as a separator, so `syrup_agent` is two terms in the
