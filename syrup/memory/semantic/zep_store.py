"""Semantic memory, as a temporal graph — Zep behind the same six methods.
    ...
    SYRUP_SEMANTIC_STORE=zep   ZEP_API_KEY=z_...

This is the most architecturally different backend in the arena, and it is the
one worth watching. Syrup's FTS5 store and Mem0 both hold facts as rows: writing
...
is the probe syrup is most likely to lose. Losing it to a system that was built
...
put in is not the row you get back. So:
  ...
  * `search()` returns graph results whose text is Zep's rendering of an edge or
    node, not the sentence Syrup wrote. Answers will read differently from the
