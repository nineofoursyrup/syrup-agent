...
from syrup.memory import consolidation, retrieval_gate
...
def bundled_skill_dirs() -> list[Path]:
    """Where the skills that SHIP with Syrup live — and why there are two answers.

    Skills are authored in `skills/` at the repo root: that is what CI
    validates and what a checkout has. But the wheel only packages the
    `syrup/` directory, so an install of the built wheel would have found
    nothing there and silently started with zero skills —
    ...
    `syrup/skills`, so an installed Syrup finds it next to the code.
...
"""How many threads `list_sessions` returns by default — a fixed-height
terminal list needs a bound a scrollable web page doesn't. One named
constant, not two independently chosen `20`s that
could quietly drift apart: `Syrup.list_threads` (syrup/app.py) imports this
same value rather than hardcoding its own default."""
...
class Memory:
    ...
    def _make_fact_store(conn, settings):
        ...
        if settings.semantic_store == "supabase":
            from syrup.memory.semantic.supabase_store import SupabaseFactStore

            return SupabaseFactStore(settings)
        if settings.semantic_store == "mem0":
            from syrup.memory.semantic.mem0_store import Mem0FactStore

            return Mem0FactStore(settings)
        if settings.semantic_store == "zep":
            from syrup.memory.semantic.zep_store import ZepFactStore

            return ZepFactStore(settings)
        if settings.semantic_store == "langmem":
            from syrup.memory.semantic.langmem_store import LangMemFactStore
    ...
    def list_sessions(self, limit: int = DEFAULT_SESSION_LIST_LIMIT) -> list[dict]:
        """One row per conversation: id, first user message (the title), message
        count, and when it started — newest first, bounded to `limit` (a
        fixed-height terminal list needs a bound a
        scrollable web page doesn't, which is why the dashboard's own history view
        reads its own unbounded `ops/dashboard.py::session_list` instead of this
        method — two callers, each reading the same table through the query shape
        that actually fits its own surface)."""
        rows = self.conn.execute(
            """SELECT session_id,
                      COUNT(*) AS messages,
                      MIN(created_at) AS started_at,
                      MAX(created_at) AS last_at
               FROM chat_log GROUP BY session_id ORDER BY last_at DESC LIMIT ?""",
            (limit,),
        ).fetchall()
        out = []
        for r in rows:
    ...
    def export_markdown(self) -> None:
        ...
        whiteboard's `~/.syrup/MEMORY.md` box is literally real, and "your memory
        ...
        lines = [
            "# Syrup memory",
            "",
            ("_A human-readable mirror of what Syrup remembers. The source of truth is "
