...
from syrup.graph.engine import Node, NodeFn, RouteFn
from syrup.loop.agent import Approver, CancelCheck, run_loop
...
def agent_node(name: str, out_key: str, *, client, model: str, system: str,
               ...
               approve: Approver | None = None, **node_kwargs) -> Node:
    ...
    `should_cancel`/`approve` are pure pass-through (#26): the node hands them
    to run_loop exactly as given and decides nothing about either — a graph
    node is still a loop turn, so it must not be a place where either gate
    can quietly go missing."""
    ...
    def run(state: dict) -> dict:
        ...
        result = run_loop(client=client, model=model, system=system,
                          ...
                          should_cancel=should_cancel, approve=approve)
