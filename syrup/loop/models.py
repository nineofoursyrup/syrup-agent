...
Pick with SYRUP_PROVIDER=anthropic|openai|gemini|deepseek|minimax|kimi|glm|openrouter
and set that provider's API key in .env. Override the model ids with SYRUP_MODEL /
SYRUP_SMALL_MODEL if the defaults below age out — they're just strings. This
matters most for openrouter: it's a single key in front of hundreds of models,
so SYRUP_MODEL=<vendor>/<model> (e.g. "google/gemini-3.5-flash") picks whichever
...
class Provider:
    ...
    # in a provider-specific env var.  SYRUP_BASE_URL remains the global custom
    # override for backwards compatibility, but must not leak across providers.
    base_url_env: str = ""
    endpoints: tuple[ProviderEndpoint, ...] = ()
    # A gateway sits in front of many vendors' catalogs behind one key
    # (openrouter, the two opencode endpoints). Its own default model/small_model
    # above name whichever vendor happens to be its $0 tier today — that is NOT
    # its family. So a SYRUP_MODEL id that happens to start with "gpt", "claude",
    # etc. is an ordinary, intended choice for a gateway, not a leaked model name
    # from switching providers. See _belongs_elsewhere().
...
PROVIDERS: dict[str, Provider] = {
    ...
    # /v1/chat/completions (they need /v1/responses), so every Syrup turn 400s on
    ...
    # checked). Override with SYRUP_SMALL_MODEL=kimi-k3 if your key is K3-only.
...
def _no_key_message(name: str, key_env: str) -> str:
    ...
    the fact that Syrup speaks to eleven providers, not one.
    """
    from syrup.config import DOTENV_PATH
    ...
    return (
        ...
        f"Switch with SYRUP_PROVIDER=<name> and that provider's key."
    )
...
def _families(provider: Provider) -> set[str]:
    ...
    a new provider is covered the moment it is added. Vendor-namespaced ids
    ("anthropic/claude-3" on openrouter) are dropped here too, but that alone
    doesn't exclude a gateway: opencode_go's own default ("deepseek-v4-flash")
    has no "/" and would otherwise register it as deepseek's "owner", shadowing
    the real deepseek provider. The caller excludes gateways outright instead.
    """
...
def _belongs_elsewhere(model: str, provider_name: str) -> bool:
    ...
    Live bug (SYRUP_PROVIDER=opencode_go, SYRUP_MODEL=gpt-5.6-luna): the family
    lookup below found "gpt" owned by the built-in "openai" provider and
    silently cleared the override, falling back to opencode_go's own default.
    But opencode_go is a GATEWAY — it legitimately serves gpt-family models
    under its own catalog, so a "gpt" prefix there is not evidence the model
    leaked in from switching away from the openai provider. Gateways are
    exempted outright rather than added to the ownership map, because they are
    the one case where "this family is owned by provider X" is true and
    ...
    if current is not None and current.is_gateway:
        return False
    owner = {f: name for name, p in PROVIDERS.items() if not p.is_gateway
...
def get_client(settings: Settings):
    ...
    if provider is None:
        raise SystemExit(f"Unknown SYRUP_PROVIDER '{settings.provider}'. "
    ...
    # A model name belongs to the provider it was configured FOR. SYRUP_MODEL and
    # SYRUP_SMALL_MODEL are global, so code that switches provider — the arena
    ...
    for attr in ("model", "small_model"):
        inherited = os.getenv(f"SYRUP_{attr.upper()}", "").strip()
    ...
    timeout = float(os.getenv("SYRUP_LLM_TIMEOUT", "120"))
