"""`python -m syrup gather` — the morning briefing, run as a graph.

    30 7 * * *  cd ~/syrup-agent && make gather

This is the ONE place where real callables meet the pure workflow in
syrup/graph/workflows/gather.py. Both the CLI and the dashboard's
...
from syrup.graph.workflows.gather import DIGEST_PROMPT, build_gather_graph

DEFAULT_TOPICS = "AI agent harness loop memory eval"


def _github(settings) -> dict:
    """Open PRs and issues. Calls syrup/tools/github.py as a LIBRARY, not through
    the registry — so a gather works with SYRUP_GH_TOOL off. That flag decides
    whether the MODEL can reach GitHub; this is our own code asking."""
    from syrup.tools import github
...
def _web(settings) -> str:
    from syrup.tools import search
...
def _calendar(settings) -> dict:
    ...
    from syrup.graph.workflows.triage import todays_events
...
def _memory(settings) -> str:
    ...
    Opens its OWN sqlite connection rather than reusing syrup.memory's. A
...
def _synthesize(syrup, state: dict) -> str:
    ...
    resp = syrup.client.messages.create(
        model=syrup.settings.model, max_tokens=1500,
...
def build_bound_graph(syrup: Syrup):
    ...
    return build_gather_graph(
        ...
        synth_fn=lambda state: _synthesize(syrup, state),
        draft_fn=lambda state: _draft(s.home, state),
    )


def run_gather(syrup: Syrup | None = None, observer=None) -> dict:
    ...
    """
    own = syrup is None
    syrup = syrup or Syrup()
    try:
        def notify(kind: str, ev: dict) -> None:
            syrup.tracer.event(kind, ev)
            if observer:
                observer(kind, ev)

        return run_graph(build_bound_graph(syrup), {}, observer=notify)
    finally:
        if own:
            syrup.close()
