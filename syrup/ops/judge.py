...
Completion (syrup.ops.scoring) is deterministic — did the right tool fire. Quality
...
pure text, no tools). Switchable per-race from the arena, or via SYRUP_JUDGE_*.
Any provider works — Syrup's OpenAI-compat client exposes the same
...
JUDGE_PROVIDER = os.getenv("SYRUP_JUDGE_PROVIDER", "openai")
JUDGE_MODEL = os.getenv("SYRUP_JUDGE_MODEL", "gpt-5.6-sol")
...
_JUDGE_SEM = threading.Semaphore(int(os.getenv("SYRUP_JUDGE_CONCURRENCY", "2")))
