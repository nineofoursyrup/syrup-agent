// syrup dashboard — formatters + chat card renderers + chatlog + streaming + send.
...
function histItem(m){
  ...
  if (m.meta) return {role:"syrup", reply:m.content, gate:m.meta.gate,
                      graph:m.meta.graph,
                      tools:m.meta.tools, iterations:m.meta.iterations,
                      latency_ms:m.meta.latency_ms, model:m.meta.model};
  return {role:"syrup", reply:m.content, historical:true};
}
...
async function sendChat(fromInput){
  ...
  const pending = {role:"syrup", pending:true, stream:"", started: Date.now()};
  CHAT.push(pending);
  syncChatLogs();
