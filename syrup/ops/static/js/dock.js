// syrup dashboard — chat sessions/history (loadThreadInto), model chip, stats toggle.
...
function applyTele(){
  const off = localStorage.getItem("syrup_tele") === "0";
...
function toggleTele(){
  const off = localStorage.getItem("syrup_tele") === "0";
  localStorage.setItem("syrup_tele", off ? "1" : "0");   // flip
  applyTele();
}
