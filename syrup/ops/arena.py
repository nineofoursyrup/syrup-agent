...
demo: `.syrup/` is never opened here.
...
Two scores, deliberately separate:
  Completion  deterministic — did the right tool fire, with the right args?
              (syrup.ops.scoring, only for prompts in the battery)
  Quality     an LLM referee's grade (syrup.ops.judge), run AFTER the race as one
              gentle pass so a burst of concurrent calls can't 429 half of them.

Results land in the arena's own JSONL scoreboard (syrup.ops.compare_history) —
...
from syrup.ops import compare_history, scoring
from syrup.ops import judge as judge_mod
from syrup.ops.pricing import cutoff_for, price_for
...
def compare_stream(message: str, specs: list, emit, judge: bool = False,
    ...
    def run(spec):
        ...
        try:
            ...
            # EACH model writes its own event to the real 'Syrup' calendar.
            settings = Settings(
                provider=provider,
                model=model,
                small_model="",
