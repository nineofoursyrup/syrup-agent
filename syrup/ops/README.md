# syrup/ops — the Eval/LLM-Ops pillar
...
whole directory and syrup still runs. That's deliberate: ops observes, it never
participates.
...
| `browser_agent.py` | The ONE shared `Syrup` behind the browser gateway + its dated chat session. |
...
| `show_trace.py` | `syrup trace` — reading those files back in the terminal. |
...
```python
from syrup.ops.browser_agent import _agent   # frozen at None forever
```
