...
2. **What you chose.** `.syrup/models.json` holds an ordered `provider:model`
...
from syrup.ops.pricing import remember_price
...
def list_models(provider: str | None = None, *, use_cache: bool = True) -> dict:
    ...
    OpenAI-compatible endpoints (OpenRouter, Gemini, any SYRUP_BASE_URL), or the
    ...
    # SYRUP_BASE_URL only applies to the provider it was set for).
    ...
    req = urllib.request.Request(url, headers={
        ...
        "User-Agent": "Mozilla/5.0 (compatible; Syrup)",
    })
    try:
...
def pinned_specs() -> list[str]:
    ...
    .syrup/models.json. The chat switcher shows exactly these. Before they've
