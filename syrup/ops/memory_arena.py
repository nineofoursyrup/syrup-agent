...
Sibling of syrup/ops/arena.py. That one holds the harness constant and varies
...
facts live. One dial each, so a result means something.
    ...
    seed conversation ──┬─→ syrup + FTS5   ──┐
    (8 messages)        ├─→ syrup + Mem0   ──┼─→ same 7 probes ─→ one scorer
                        └─→ syrup + Zep    ──┘

Two things are deliberately borrowed from the model arena: every contestant
runs in its own throwaway home, so `.syrup/` is never opened; and the results
...
# the file is swappable and syrup keeps the mechanism, not the content.
_EXAMPLE = Path(__file__).resolve().parents[2] / "evals" / "memory_arena.json"
PROBES_ENV = "SYRUP_MEMORY_PROBES"
...
def fixture_path() -> Path:
    """Where the probes are coming from. SYRUP_MEMORY_PROBES wins, so a run can
...
def probe_files() -> list[dict]:
    ...
    no reason to need one. Drop a file in `.syrup/probes/` and it appears.
    """
...
def score(answer: str, probe: dict, retrieved: bool | None = None) -> tuple[str, bool, str]:
    ...
    `retrieved` is whether the backend went to memory for this probe. Only syrup
...
# --- the runner -------------------------------------------------------------
# Everything above is pure. This part costs money: it stands up a real Syrup per
...
def run_arena(backends: list[str], track: str, emit, fixture: dict | None = None,
    ...
    One agent, one model, one loop. The ONLY variable is `SYRUP_SEMANTIC_STORE`.
    ...
    run can store, consolidate and retrieve without ever opening `.syrup/`. The
    ...
    for backend in backends:
        ...
        try:
            ...
            app = Syrup(settings=Settings(home=home, semantic_store=backend,
...
def store_contents(limit: int = 8, only: str = "") -> list[dict]:
    ...
    for key in _available_backends():
        ...
        # saying so, "53 vs 17" reads as "syrup remembers more", when it only
        # means syrup has been used and the others were connected yesterday.
...
def _store_note(key: str) -> str:
    ...
    """
    if key == "langmem" and not os.getenv("SYRUP_LANGMEM_POSTGRES", "").strip():
        return ("in-memory store — contents live inside the process that wrote them "
                "and cannot be read back here. Set SYRUP_LANGMEM_POSTGRES to persist.")
    return ""


def _conn_for(key: str, settings):
    """Only the sqlite store needs a connection; the hosted ones ignore it. The
    live .syrup/state.db is opened READ-ONLY here — this page reports, it never
...
def _available_backends() -> list[str]:
    ...
    from syrup.integrations import IntegrationState, list_integrations
