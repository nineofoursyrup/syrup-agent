"""The Excalidraw whiteboard style, locked in code.

Every board Syrup generates should be indistinguishable from the hand-drawn
reference boards (~/Developer/Excalidraw) so it's ready to use directly. The values
here were reverse-engineered from those references, not invented:
...
def labeled_box(x, y, w, h, label, *, color="plain", size=FS_BOX, rounded=True,
    ...
    fill mode (the house idiom is mostly WHITE shapes, color in the stroke):
...
def source_label(x, y, s, *, frame=None, group=None):
    """Small grey citation chip, per the source-label rule."""
...
def socials_block(x, y):
    """The socials block for shareable charts (top-right): GitHub + email."""
    return [
        text(x, y, "GitHub  @nineofoursyrup", size=FS_BODY, color=INK),
        text(x, y + 24, "Email   nineofoursyrup@gmail.com", size=FS_BODY, color=INK),
    ]


def watermark(x, y):
    return text(x, y, "@nineofoursyrup", size=FS_BODY, color=PAL["grey"][1])
...
def socials_logos(x, y, *, size=44, row=58, text_dx=64):
    """The socials block WITH the real GitHub logo, plus an email line."""
    out = [image("github.png", x, y, size, size)]
    out.append(text(x + text_dx, y + size / 2 - FS_BODY * LINE_HEIGHT / 2,
                    "@nineofoursyrup", size=FS_BODY, color=INK))
    out.append(text(x, y + row, "nineofoursyrup@gmail.com", size=FS_BODY, color=INK))
    return out
...
def document(elements):
    return {
        "type": "excalidraw", "version": 2, "source": "syrup/ops/whiteboard",
        "elements": elements,
