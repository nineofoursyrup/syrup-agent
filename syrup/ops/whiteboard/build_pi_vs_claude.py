...
aligned rows each, then the syrup punchline. Facts are sourced on the board
...
Run:  python -m syrup.ops.whiteboard.build_pi_vs_claude
"""
...
ROWS = [
    ...
    ("SUB-AGENTS",
     ...
     "refused — spawn pi yourself: tmux,\nor syrup's delegate_task (that's us)",
     "plain"),
    ("MCP",
...
def build() -> list:
    ...
    e += S.labeled_box(80, band, 2410, 96,
                       "syrup uses BOTH: Claude Code writes syrup's code - pi is syrup's delegate_task sub-agent,\n"
