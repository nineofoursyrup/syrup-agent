"""Generate the current Syrup architecture whiteboard.

Run:
    python -m syrup.ops.whiteboard.build_syrup_architecture
...
BOARD = ROOT / "docs" / "whiteboards" / "syrup-architecture.excalidraw"
SVG_PROXY = Path("/tmp/syrup-architecture.svg")
...
def build_elements() -> list[dict]:
    ...
    # Title and identity.
    a(S.text(60, 36, "Syrup — one turn through the whole system", size=S.FS_TITLE))
    ...
    # ------------------------------------------------------------------ Harness
    a(S.boundary(40, 190, 2440, 470, "HARNESS — syrup/app.py", color="red"))

    a(S.pill_header(80, 250, 330, "1  GATEWAYS", color="grey"))
    a(S.labeled_box(80, 320, 330, 210,
                    "text in / text out\n\nCLI · Dashboard · Voice\nTelegram · Discord\n\nsyrup/gateway/ + ops/dashboard.py",
    ...
    # ------------------------------------------------------------------- Memory
    a(S.boundary(40, 710, 1490, 580, "MEMORY — syrup/memory/", color="green"))
    ...
    # -------------------------------------------------------------- Eval / Ops
    a(S.boundary(1590, 710, 890, 580, "EVAL / LLM OPS — syrup/ops/ + evals/", color="blue"))
    ...
    a(S.source_label(60, 1330,
                     "source: syrup/app.py, runtime/session.py, loop/agent.py, memory/*, tools/*, ops/* — 2026-07-27"))
