...
by :mod:`syrup.integrations`. This module retains the Experimental and
...
def settings_info() -> dict:
    ...
    # Resolve the model the same way the loop does. `syrup/loop/models.py` fills a
    # blank SYRUP_MODEL from the provider's default at build time, so the agent is
...
def apply_settings(payload: dict) -> dict:
    ...
    toggles = (("experimental", "SYRUP_EXPERIMENTAL"), ("graph_workflows", "SYRUP_GRAPH_WORKFLOWS"))
