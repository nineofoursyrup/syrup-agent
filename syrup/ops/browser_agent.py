"""The browser gateway's agent — one Syrup, shared by every tab.
...
def resume_or_new_session(conn) -> str:
    ...
    idle_min = int(os.getenv("SYRUP_SESSION_IDLE_MINUTES", "60"))
...
def get_agent():
    ...
    if _agent is None:
        ...
        _agent = Syrup(settings=settings, conn=conn)
...
def maybe_rotate_session(agent) -> None:
    ...
    current session's newest message is older than SYRUP_SESSION_IDLE_MINUTES
...
def rebuild() -> str | None:
    ...
    with agent_lock:
        ...
        try:
            ...
            settings.ensure_home()
            conn = connect(settings.home, check_same_thread=False)
            fresh = Syrup(settings=settings, conn=conn)
            # Carry the CONVERSATION across the swap. A settings change swaps the
            # brain, not the thread — but a brand-new Syrup starts on the eternal
