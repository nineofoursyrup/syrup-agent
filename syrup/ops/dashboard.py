"""Dashboard — every pillar on one local page. Zero new dependencies.
    ...
    make dashboard        # → http://localhost:9049

One stdlib HTTP server reading the files Syrup already writes:
...
from syrup.integrations import (
    apply_integration,
    apply_provider,
...
from syrup.ops import browser_agent, commands, compare_history
from syrup.ops.arena import (
    compare_clear,
    compare_delete_run,
    compare_regrade,
    compare_stream,
...
from syrup.ops.browser_agent import agent_lock, dash_session, get_agent, maybe_rotate_session
from syrup.ops.catalog import list_models
from syrup.ops.pricing import price_for, usage_summary
from syrup.ops.settings_api import apply_settings, pin_action, settings_info
from syrup.ops.tracing import TraceEncodingError, iter_trace_lines
from syrup.runtime.session import mint_session_id
...
def collect() -> dict:
    ...
    from syrup.memory.procedural.loader import SkillLoader
    ...
    from syrup.graph.workflows.gather import gather_topology
    from syrup.graph.workflows.triage import triage_topology
    ...
    return {
        ...
        "graph": {
            ...
            # `syrup gather` is a routine you run yourself and ignores this flag
...
def _rel_to_home(path, home) -> str:
    """Path relative to SYRUP_HOME if it lives there, else the repo-relative
...
def session_list(conn) -> list[dict]:
    ...
    for g in groups:
        ...
        preview = ""
        if last:
            preview = ("you: " if last["role"] == "user" else "syrup: ") + last["content"][:80]
        out.append({"id": sid,
...
def tools_info() -> dict:
    ...
    if live is not None:
        ...
        from syrup.tools import calendar, memory_admin, messages, notes, search
        ...
        if settings.experimental:
            ...
            # switch did nothing.
            from syrup.tools import experimental as experimental_tools
    ...
    from syrup.tools.experimental import PLANNED
...
def transcribe_audio(raw: bytes) -> dict:
    ...
    with _whisper_lock:
        if _whisper is None:
            _whisper = WhisperModel(os.getenv("SYRUP_WHISPER_MODEL", "base"), compute_type="int8")
...
def _editor_cmd() -> list[str] | None:
    """The user's code editor CLI: $SYRUP_EDITOR, then cursor, then code."""

    custom = os.getenv("SYRUP_EDITOR")
...
def reveal_path(rel: str) -> dict:
    """Open a file/folder under SYRUP_HOME — in the user's code editor if one
    is on PATH (cursor/code/$SYRUP_EDITOR), otherwise reveal in Finder.
    Restricted to paths inside SYRUP_HOME."""
    import subprocess
    import sys
    ...
    if target != home and home not in target.parents:
        return {"error": "path is outside the .syrup home"}
...
def memory_action(payload: dict) -> dict:
    ...
    from syrup.memory.episodic.store import SqliteEpisodeStore
    from syrup.memory.semantic.store import SqliteFactStore
    ...
    if action == "save_skill":
        ...
        from syrup.memory import bundled_skill_dirs
        from syrup.memory.procedural.loader import _parse_text
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/data":
            ...
            from syrup.ops import memory_arena
            ...
            try:
                ...
                # it under mem0 showed you syrup's local facts and said nothing.
            ...
            # which a wheel does not ship — a pip-installed Syrup answers with
...
def main() -> None:
    # Port precedence: SYRUP_DASHBOARD_PORT, then the conventional PORT (used by
    # deploy platforms and IDE preview panes), then 9049. If it's taken, walk on.
    base = int(os.getenv("SYRUP_DASHBOARD_PORT") or os.getenv("PORT") or PORT)
    for port in range(base, base + 10):  # walk past a busy port instead of crashing
        ...
        from syrup.gateway.dingtalk import start_in_background as start_dingtalk
        from syrup.gateway.discord import start_in_background as start_discord
        from syrup.gateway.feishu import start_in_background as start_feishu
        from syrup.gateway.supervisor import GatewaySupervisor
        from syrup.gateway.telegram import start_in_background as start_telegram
        from syrup.gateway.whatsapp import start_in_background as start_whatsapp
        from syrup.integrations import (
            INTEGRATIONS,
        ...
        supervisor = GatewaySupervisor(
            {"telegram": start_telegram, "discord": start_discord, "whatsapp": start_whatsapp,
             "dingtalk": start_dingtalk, "feishu": start_feishu},
        ...
        print(f"Syrup dashboard → http://localhost:{port}  (Ctrl-C to stop)")
        try:
