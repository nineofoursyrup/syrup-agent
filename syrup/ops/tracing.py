...
1. JSONL, always on: every turn appends readable lines to
   .syrup/traces/<date>.jsonl. A trace is just "what happened, in order" —
...
2. OpenTelemetry spans, when OTEL_EXPORTER_OTLP_ENDPOINT is set: the same
   events as a span tree any OTel backend can render. For a local dashboard:
       ...
       pip install 'syrup-agent[tracing]'
       phoenix serve                                # localhost:6006
       OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317 python -m syrup
...
class TraceEncodingError(UnicodeError):
    ...
    def __init__(self, path: Path):
        ...
        super().__init__(
            ...
            "Syrup version using the Windows system encoding. Move it out of the traces directory "
            "and keep it as a backup, then restart Syrup if it is today's trace. The file "
            "was not modified."
        )
...
class Tracer:
        ...
        if self._otel_tracer and self._span_ctx is not None:
            with self._otel_tracer.start_as_current_span(
                ...
                attributes={
                    ...
                    **{f"syrup.{k}": json.dumps(v, default=str) for k, v in event.items()},
                },
            ):
                pass
    ...
    def turn(self, user_message: str):
        ...
        if self._otel_tracer:
            with self._otel_tracer.start_as_current_span(
                "agent_run",
                attributes={"openinference.span.kind": "AGENT", "syrup.user_message": user_message},
            ) as span:
                self._span_ctx = span
                try:
                    yield self
    ...
    def end_turn(self, reply: str, iterations: int, cancelled: bool = False) -> None:
        # `cancelled` rides on the same turn_end record rather than a separate
        # event type — a reader scanning turn_end lines sees honestly whether
        # each one finished or was stopped, without a second thing to check.
