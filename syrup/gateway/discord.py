"""Discord gateway — message Syrup from a direct message or a server channel.

READ THIS BEFORE PUTTING THE BOT IN A SHARED SERVER.

This gateway runs YOUR agent: your memory (`.syrup/state.db`), your SOUL.md, your
...
Setup:
  ...
  4. Open it up only as far as you actually need (all optional):
     ...
     DISCORD_HOME              a SEPARATE .syrup for the bot. Use this whenever
...
def _strip_mention(content: str, bot_id: str) -> str:
    """Drop the '@Syrup Agent' prefix so the model sees the question, not the ping."""
    for form in (f"<@{bot_id}>", f"<@!{bot_id}>"):
        content = content.replace(form, " ")
    return content.strip()


def _build_agent() -> Syrup:
...
def describe_posture() -> str:
    ...
    home = os.getenv("DISCORD_HOME", "").strip() or ".syrup — YOUR personal memory"
...
def main() -> None:
    try:
        ...
        raise SystemExit("Discord extra not installed: pip install 'syrup-agent[discord]'")
    ...
    try:
        ...
        print("Syrup is listening on Discord. Ctrl-C to stop.")
...
def start_in_background() -> DiscordHandle | None:
    ...
    try:
        ...
        print("(discord) DISCORD_BOT_TOKEN is set but the extra isn't installed — "
              "pip install 'syrup-agent[discord]'")
        return None
