...
only moves words in and out (that's the whole point of the gateway box).
  ...
  voice  macOS `say` with a British voice by default (zero setup), or the
         ...
         then set SYRUP_TTS=kokoro  (SYRUP_VOICE=bm_george / bm_fable / ...)
...
from syrup.gateway.cli import _observer  # show gate/tool lines in voice mode too
...
class Ears:
    def __init__(self, model_size: str | None = None):
        ...
        self.model = WhisperModel(
            model_size or os.getenv("SYRUP_WHISPER_MODEL", "base"),
            compute_type="int8",
        )

    def transcribe(self, audio, language: str | None = None) -> str:
        segments, _ = self.model.transcribe(
            audio, language=language or os.getenv("SYRUP_WHISPER_LANG")
        )
...
class Mouth:
    ...
    def __init__(self):
        self.engine = os.getenv("SYRUP_TTS", "").strip().lower()
        self.voice = os.getenv("SYRUP_VOICE", "")
        if not self.engine:
    ...
    def speak(self, text: str) -> None:
        ...
        if self.engine == "kokoro":
            ...
            print("(no TTS engine on this platform — set SYRUP_TTS=kokoro)")


def matches_wake(text: str, wake_word: str) -> bool:
    """Does a transcript contain the (customizable) wake word?

    Fuzzy on purpose: Whisper hears "syrup syrup" as "syrupsyrup", "Syrup, syrup!",
    "sylrup syrup" — or transcribes it as Japanese kana わくわく (the first live
    test!). So: `wake_word` accepts comma-separated variants across scripts
    ("syrup syrup,わくわく"), normalization keeps kana AND CJK, and matching is
...
def _mic_threshold() -> float:
    ...
    SYRUP_MIC_THRESHOLD (lower if it never hears you, higher if it
    wakes on room noise)."""
    return float(os.getenv("SYRUP_MIC_THRESHOLD", "0.005"))
...
def wake_loop(syrup: Syrup, mouth: Mouth, wake_word: str) -> None:
    ...
    - The mic buffer is drained after Syrup speaks, so it doesn't wake on
    ...
    ack = os.getenv("SYRUP_WAKE_ACK", "Yes?")
    followup = float(os.getenv("SYRUP_FOLLOWUP_SECONDS", "8"))  # stay open, Siri-style
    block = SAMPLE_RATE // 10
    # Pin the scout's transcription language to match the wake word's script —
    # otherwise Whisper hears "syrup syrup" and helpfully writes わくわく, which
    # a latin wake word never matches. Commands after wake still auto-detect.
    wake_lang = os.getenv("SYRUP_WAKE_LANG") or ("en" if wake_word.isascii() else None)
        ...
        while True:
            ...
            if not matches_wake(heard_scan, wake_word):
                ...
                if heard_scan:  # near-misses belong in the trace (wake tuning!)
                    syrup.tracer.event("wake_scan", {"heard": heard_scan, "matched": False})
                continue
            ...
            # for a follow-up for `followup` seconds — no need to say the wake
            # word again (like Siri). A quiet stretch drops back to wake-word mode.
            while True:
                heard = ears.transcribe(record_command(stream))
                if heard:
                    print(f"you › {heard}")
                    result = syrup.respond(heard, observer=_observer, source="voice")
...
def main() -> None:
    ...
    syrup = Syrup()
    syrup.session.session_id = "voice"   # its own conversation thread in the inbox
    mouth = Mouth()

    # Hands-free by default: always-listening for "susususu syrupnova". The default
    # packs in "supernova" variants too — it's a real English word, and the tiny
    # scanner's Whisper model is very likely to normalize the made-up "syrupnova"
    # to it, so it triggers reliably either way.
    # Set SYRUP_WAKE_WORD="" for push-to-talk instead.
    wake_word = os.getenv(
        "SYRUP_WAKE_WORD",
        "susususu syrupnova,susususu supernova,su su su syrup nova,su su su supernova,"
        "syrupnova,supernova,syrup nova",
    ).strip()
    if wake_word:
        try:
            wake_loop(syrup, mouth, wake_word)
    ...
    ears = Ears()
    print("Voice Syrup ready. Press Enter to talk, Ctrl-C to quit.")
    while True:
        try:
            input("\npress Enter to talk… ")
