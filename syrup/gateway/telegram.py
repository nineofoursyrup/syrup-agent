...
Setup (2 minutes, free):
  ...
  3. Set TELEGRAM_ALLOWED_USER=<your numeric id> (message @userinfobot to get
     it) so ONLY you can talk to your Syrup. Comma-separate for several people.
...
from syrup.gateway.cli import _observer  # mirror gate/tool activity on the laptop terminal
...
def _new_runner() -> GatewayAgentRunner:
    return GatewayAgentRunner(
        Syrup, session_id="telegram", source="telegram", observer=_observer
    )
...
def _build_app(token: str, allowed: str = "", runner: GatewayAgentRunner | None = None):
    ...
    gateway and the background poller `syrup dashboard` starts.
    ...
    async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        if allowed_ids and str(update.effective_user.id) not in allowed_ids:
            await update.message.reply_text("This Syrup serves someone else. Run your own!")
            return
...
def main() -> None:
    try:
        ...
        raise SystemExit("Telegram extra not installed: pip install 'syrup-agent[telegram]'")
    ...
    try:
        ...
        print("Syrup is listening on Telegram — message your bot. Ctrl-C to stop.")
        print(posture())
        app.run_polling()
    finally:
        runner.close()
...
def start_in_background() -> TelegramHandle | None:
    """Start the Telegram poller on a daemon thread — so `syrup dashboard` runs
    ...
    try:
        ...
        print("(telegram) TELEGRAM_BOT_TOKEN is set but the extra isn't installed — "
              "pip install 'syrup-agent[telegram]'")
        return None
    ...
    def on_poll_error(exc: Exception) -> None:
        ...
        if isinstance(exc, Conflict) and not warned["conflict"]:
            ...
            print("(telegram) another instance is already running this bot — the dashboard's "
                  "Telegram stays idle. Stop the other `syrup telegram` and restart to use it here.")
