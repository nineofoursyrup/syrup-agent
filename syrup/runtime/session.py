...
away. What persists lives in syrup/memory. Working memory =

    system prompt (SOUL.md)            ← who Syrup is
  + durable facts & episodes           ← what Syrup remembers (gated!)
...
DEFAULT_SOUL = """\
You are Syrup, a personal assistant running locally on your user's laptop.
...
- Be honest about where things live. Every tool's output states exactly where
  ...
  .syrup/state.db) — relay that truthfully, and never claim something synced
...
def load_soul(settings: Settings) -> str:
    ...
    changes who your Syrup is — that's procedural memory at its simplest."""
...
def mint_session_id() -> str:
    """Generate a new thread identifier, timestamped down to the second so
    tags sort and read chronologically. This is THE one implementation of
    "what a freshly-started thread is called" — the dashboard's own "New
    chat" button (syrup/ops/dashboard.py's `session_action`, action="new")
    calls this rather than formatting its own copy, so a thread minted from
    any surface is legible in every other one (one naming convention, not
    several that could drift). Distinct from `syrup/ops/browser_agent.py`'s
    "dashboard-" prefixed tags, which name a *surface* auto-tagging its own
    session on launch/idle-rotation — a different concept from a person
    explicitly asking to set the current thread aside and start fresh."""
    return datetime.now().strftime("s-%Y%m%d-%H%M%S")
...
class Session:
    ...
    def __init__(self, settings: Settings, memory=None, session_id: str = "default"):
        ...
        self.memory = memory  # syrup.memory.Memory (None until Phase-2 wiring)
    ...
    def build_system(self, user_message: str, notify=None) -> str:
        ...
        parts = [load_soul(self.settings),
                 ...
                 f"'{self.settings.provider}' provider, inside Syrup, a local-first "
                 f"open-source agent harness (github.com/nineofoursyrup/syrup-agent).")]
    ...
    def add_exchange(self, user_message: str, reply: str, tool_calls: list | None = None,
                     source: str = "cli", meta: dict | None = None, cancelled: bool = False) -> None:
        ...
        `cancelled` marks a turn stopped mid-flight (#22): the reply may be
        empty (stopped before the model said anything) and tool_calls may be
        empty too (stopped before any tool dispatched). Either way the record
        must stay non-empty — providers reject an empty assistant content
        block outright, and that block gets resent as history on the very
        next turn. Whatever text or tool activity exists is still recorded
        first (persist, don't discard); this placeholder only fills the gap
        when both are missing."""
        record = reply
        if tool_calls:
            summary = "; ".join(f"{c['tool']}({c['args']}) -> {c['output']}" for c in tool_calls)
            record = f"{reply}\n[tools used: {summary}]" if reply else f"[tools used: {summary}]"
        if not record:
            record = "[cancelled before replying]" if cancelled else "[no reply]"
