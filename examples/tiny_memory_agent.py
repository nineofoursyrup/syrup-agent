...
to /usr/bin/python3, which does not have syrup installed, and the failure is a
...
Your keys are picked up automatically — syrup/config.py calls find_dotenv from
the working directory, so no exports are needed.

WHY THIS FILE EXISTS

The full Syrup loop sends every registered tool schema on every turn. Measured on
...
It reads the SAME store as your live agent (`SYRUP_SEMANTIC_STORE` picks the
...
from syrup.memory.retrieval_gate import should_retrieve
...
def demo_store(settings, client):
    ...
    seeds it from evals/memory_arena.json, and never opens .syrup at all.
    """
    import json
    import tempfile
