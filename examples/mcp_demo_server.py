"""A tiny, self-contained MCP server — the demo connector for syrup-agent.
...
so the connector story runs with zero extra installs:
    ...
    pip install -e '.[mcp]'
    cp examples/mcp.demo.json .syrup/mcp.json
...
that's the whole point: connectors plug in without changing Syrup's code.
"""
...
if __name__ == "__main__":
    mcp.run()  # stdio transport — how Syrup's MCPBridge talks to it
