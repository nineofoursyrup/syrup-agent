...
name: excalidraw
description: >
  Generate an Excalidraw whiteboard in a hand-drawn style (Excalifont,
  roughness 1, green signature, socials + watermark, source labels). Use whenever
  the user wants a whiteboard, diagram, teaching board, or "chart" for
  the docs/whiteboards gallery.
---

# Excalidraw whiteboards, this project's way

Boards are generated from code so they match the reference boards in `~/Developer/Excalidraw`
exactly — ready to use directly. Never hand-write `.excalidraw` JSON; use the
...
```python
from syrup.ops.whiteboard import style as S
...
See `syrup/ops/whiteboard/build_k3_tutorial.py` for a full two-board example.

## Palette (name → fill/stroke), meaning follows the project's color system
...
3. Shareable boards get the socials block + watermark (`style.socials_block`/`style.watermark`).
...
Fix both before showing it — they're the first things a viewer notices.
