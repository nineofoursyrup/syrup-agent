 ...
 *                     command that tries to wipe syrup's runtime data (.syrup, the
...
// Team Rocket guard: hands off the Pokemon Center (.syrup runtime data).
// Blocks the exact mistake syrup's own rules forbid — wiping .syrup or `rm -rf /`.
const ROCKET = /\brm\b[^\n]*\s(-[rRfF]+\s+)?(\.syrup(\/|\b)|\/\s|~\/?\s*$)/;
	...
	pi.on("tool_call", async (event) => {
		...
		if (ROCKET.test(command)) {
			return {
				block: true,
				reason:
					"Team Rocket guard: that command targets the Pokemon Center (.syrup " +
